...
const SupportingDocumentsSection: React.FC<SupportingDocumentsSectionProps> = ({
  ...
  const handleSupportingDocUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && selectedDocType) {
      if (Array.isArray(files.supportingDocs) && files.supportingDocs.length >= 6) {
        alert('You can upload up to 6 supporting documents.');
        e.target.value = '';
        return;
      }
  ...
  return (
    <div className="border-b pb-8">
      <h2 className="text-xl font-semibold mb-6 text-gray-800">Supporting Documents</h2>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-green-500 transition-colors">
              ...
              <label htmlFor="supportingDocs" className="cursor-pointer">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select type above, then upload (max 6 files)
                </p>
              </label>
            </div>
          </div>
